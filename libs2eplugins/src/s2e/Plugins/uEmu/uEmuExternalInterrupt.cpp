...
#include "uEmuExternalInterrupt.h"

namespace s2e {
namespace plugins {

S2E_DEFINE_PLUGIN(uEmuExternalInterrupt, "trigger and record external interrupts", "uEmuExternalInterrupt");

class uEmuExternalInterruptState : public PluginState {
private:
...
public:
    ...
    virtual ~uEmuExternalInterruptState() {
    }

    static PluginState *factory(Plugin *, S2EExecutionState *) {
        return new uEmuExternalInterruptState();
    }

    uEmuExternalInterruptState *clone() const {
        return new uEmuExternalInterruptState(*this);
    }
...
void uEmuExternalInterrupt::initialize() {
    s2e()->getCorePlugin()->onTranslateBlockStart.connect(
        sigc::mem_fun(*this, &uEmuExternalInterrupt::onTranslateBlockStart));
...
void uEmuExternalInterrupt::onTranslateBlockStart(ExecutionSignal *signal, S2EExecutionState *state, TranslationBlock *tb,
                                              uint64_t pc) {
    signal->connect(sigc::mem_fun(*this, &uEmuExternalInterrupt::onBlockStart));
}

std::map<uint32_t, bool> uEmuExternalInterrupt::setActiveIrqs(std::vector<uint32_t> irqs_bitmap) {
    std::map<uint32_t /* external irq no */, bool /*enable*/> active_irqs;
...
void uEmuExternalInterrupt::onBlockStart(S2EExecutionState *state, uint64_t pc) {
    DECLARE_PLUGINSTATE(uEmuExternalInterruptState, state);
